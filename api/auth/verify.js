...
export default async function handler(req, res) {
  ...
  if (teams) {
    ...
    if (entry) {
      const [, config] = entry;
      return res.status(200).json({ valid: true, team: config.team || null, players: config.players || [], settings: config.settings || null, squadNumbers: config.squadNumbers || null });
    }
